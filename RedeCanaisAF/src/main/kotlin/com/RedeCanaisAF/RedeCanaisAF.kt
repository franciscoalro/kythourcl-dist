...
class RedeCanaisAF : MainAPI() {
    ...
    private suspend fun requestDoc(url: String, referer: String = "$mainUrl/"): Document {
        ...
        return try {
            ...
            val resp = try {
                ...
                Log.w(TAG, "[REQ#$reqId] app.get falhou (${e.message}) -> acionando Fallback WebView imediatamente")
                null
            }

            var finalDoc = resp?.document ?: org.jsoup.Jsoup.parse("<html><body></body></html>", url)
            val initialLinks = resp?.document?.select("a[href]")?.size ?: 0

            val isBlocked = resp == null || (resp?.code != 200) || initialLinks == 0 || (resp?.text?.contains("Just a moment...", true) == true)
            if (isBlocked) {
    ...
    private fun parseCard(element: Element): SearchResponse? {
        ...
        val container = if (element.hasClass("pm-video-thumb") && parent != null && parent.selectFirst("h3, .entry-title, .pm-video-attr-title, img") != null) {
            parent
        } else {
            element
        }

        val aTag = container.selectFirst("h3 a, .entry-title a, a[href*='_']:not([href*='#']):not([class*='watch-later']):not([class*='pm-watch-later']), a[href*='.html']:not([href*='#']):not([class*='watch-later']):not([class*='pm-watch-later'])")
            ?: container.selectFirst("a:not([href*='#']):not([class*='watch-later']):not([class*='pm-watch-later'])")
            ?: (if (element.tagName() == "a") element else null)
            ?: return null

        val link = aTag.attr("href").ifBlank { return null }
        if (link.contains("browse-") || link.contains("category") || link.startsWith("#") || link.contains("modal-login")) {
            return null
        }
        val fullUrl = fixUrl(link)

        var rawTitle = ""
        val titleEl = container.selectFirst("h3 a, h3, .entry-title a, .entry-title, .pm-video-attr-title a, .pm-video-attr-title, .title")
        if (titleEl != null) {
            rawTitle = titleEl.text().ifBlank { titleEl.attr("title") }
        }
        if (rawTitle.isBlank() || rawTitle.equals("Watch Later", ignoreCase = true)) {
            val img = container.selectFirst("img")
            rawTitle = img?.attr("alt")?.ifBlank { img.attr("title") }.orEmpty()
        }
        if (rawTitle.isBlank() || rawTitle.equals("Watch Later", ignoreCase = true)) {
            rawTitle = aTag.attr("title").ifBlank { aTag.text() }
        }

        val cleanTitle = cleanMediaTitle(rawTitle)
        if (cleanTitle.isBlank() || cleanTitle.equals("Watch Later", ignoreCase = true)) return null

        val rawPoster = extractPosterFromCardElement(container)
        val poster = optimizePosterUrl(rawPoster)

        val isSeries = isSeriesUrlOrTitle(fullUrl, cleanTitle)
        val tvType = determineTvType(fullUrl, emptyList(), isSeries)

        val qualityText = container.selectFirst(".pm-label-hd, .label-hd, .quality")?.text().orEmpty()
